...
public class GetPlayerProfileService implements GetPlayerProfileUseCase {
    ...
    public PlayerProfileResponseDto getPlayerById(Long id) {
        ...
        return entity.map(PlayerProfileResponseDto::from).orElseThrow(() -> new PlayerProfileNotFoundException("No players found with id " + id));
    }
    ...
    public PlayerProfileResponseDto getPlayerByName(String name) {
        ...
        return entity.map(PlayerProfileResponseDto::from).orElseThrow(() -> new PlayerProfileNotFoundException("No players found with name " + name));
    }
}
